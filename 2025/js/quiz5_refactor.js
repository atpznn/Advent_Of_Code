const MAX_CHUNKED = 20000;
function eachArrayNeedToCreate(from, to) {
  return Array.from(
    { length: Math.ceil((to - from + 1) / MAX_CHUNKED) },
    (_, index) => {
      const i = (index + 1) * MAX_CHUNKED;
      return i > to ? i - to : MAX_CHUNKED;
    }
  );
}
...
function solveUnit(freshIds, spoiledItems) {
  return freshIds.reduce((_, freshId) => {
    if (freshId == spoiledItems) return true;
    return false;
  }, false);
...
function eachFreshId(freshIdChecked, spoiledItem) {
  return freshIdChecked.reduce((_, freshIds) => {
